...
def main():
    ...
    from predict import predict_group_matches
    ...
    print("\n[3/7] Calculando ratings Elo sobre todo el histórico…")
    ...
    print("\n[4/7] Ajustando modelo de goles (Poisson + Dixon-Coles)…")
    model = GoalModel.fit(res["calibration"])
    print(f"  a={model.a:.4f}  b={model.b:.6f}  rho={model.rho:.4f}")

    print("\n[5/7] Prediciendo los 72 partidos de fase de grupos…")
    fixtures = load_fixtures()
    pred_df = predict_group_matches(ratings, model, fixtures)
    report.write_group_predictions(pred_df)
    print(f"  {len(pred_df)} partidos predichos.")

    print(f"\n[6/7] Simulando {args.sims:,} torneos completos (Monte Carlo)…")
    ...
    print("\n[7/7] Generando informes en output/…")
    report.write_ratings(ratings_df)
    report.write_forecast(sim_df)
    report.write_stats()
    report.write_summary(sim_df, ratings_df, model, args.sims)
    print("  Escritos: team_ratings.csv, group_stage_predictions.{csv,md}, "
          "tournament_forecast.csv, team_historical_stats.csv, PREDICCIONES.md")
